...
#### **/api/restockOrder/:id/:supplierId/skuItems**
- **Add a non empty list of skuItems to a restock order, given its id and the supplierId. If a restock order has already a non empty list of skuItems, merge both arrays**
- **Request header** has a line: `Content-Type: application/json` and req.params.id to retrieve id.
